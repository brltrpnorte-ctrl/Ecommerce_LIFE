...
$patterns = @(
    ...
    @{ Name = "Admin Token assignment"; Regex = 'AUTH_TOKEN\s*=\s*[A-Za-z0-9_-]{20,}' }
)
...
foreach ($pattern in $patterns) {
    ...
    foreach ($m in $matches) {
        ...
        if ($m.Line -match "REPLACE_WITH|YOUR_PROJECT|YOUR_POOLER_HOST|YOUR_BACKEND_PROJECT|<ADMIN_TOKEN>|<SEU_TOKEN|user:password@|localhost/ecommerce_life|ecommerce_user:ecommerce_secure_password@") { continue }
